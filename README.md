...
> **Note:** Before installing, go to **Settings → Security → Install unknown apps** and enable it for your browser or file manager. CineTrekker requires **Android 5.0 (Lollipop)** or higher.
...
| 🔒 **Secure Auth** | OAuth 2.0 via Supabase with session persistence |
...
│   ├── auth/         # OAuth 2.0 auth controller & deep link handler
│   ├── constants/    # Environment variables & app-wide constants
│   ├── errors/       # Crash fallback & error boundary
│   ├── storage/      # Local cache & media list persistence
...
**Tech Stack:**
- **State Management**: [Riverpod](https://riverpod.dev) (code-gen, AsyncNotifier)
- **Navigation**: [GoRouter](https://pub.dev/packages/go_router)
- **Backend**: [Supabase](https://supabase.com) (Auth, Database, REST)
- **Movie Data**: [TMDB API](https://www.themoviedb.org/documentation/api)
- **Local Storage**: SharedPreferences + custom cache manager
- **Animations**: Rive, Lottie, and custom BouncyPressable
...
3. **Fill in your environment variables** in `.env`:
   ...
   ```
   > You will need a [Supabase](https://supabase.com) project and a [TMDB API key](https://www.themoviedb.org/settings/api).
...
| `API_BASE_URL` | Base URL for the CineTrekker backend | ✅ |
...
| [Release](.github/workflows/release.yml) | Push tag `v*` | Builds APK + AAB, creates GitHub Release, uploads APK |
...
| File | Purpose |
|---|---|
| `lib/main.dart` | App entry point, crash boundary, Riverpod setup |
| `lib/app.dart` | MaterialApp, GoRouter, theme wiring |
| `lib/core/auth/auth_controller.dart` | OAuth 2.0 session management |
...
| `lib/core/api/supabase_rest_api.dart` | Supabase backend client |
